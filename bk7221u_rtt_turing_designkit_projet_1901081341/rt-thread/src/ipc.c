...
rt_size_t rt_sem_get_value(rt_sem_t sem)
{
	rt_size_t count;	
    register rt_base_t temp;
 ...
 * @return the error code
 */
rt_err_t rt_mq_send_unique(rt_mq_t mq, void *buffer, rt_size_t size)
{
	void *msg_node;
	rt_int32_t cmp_ret;
	rt_uint32_t hit_flag = 0;
    register rt_ubase_t temp;
	...
	msg_node = mq->msg_queue_head;
	while (msg_node != RT_NULL)
	{
		msg = (struct rt_mq_message *)msg_node;
		
		cmp_ret = rt_memcmp(msg + 1, buffer, size);
		if(0 == cmp_ret)
		{
			hit_flag = 1;
			break;
