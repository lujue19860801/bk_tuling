...
extern const airkiss_config_t airkiss_cfg_ptr;
...
static void airkiss_lan_handler(void *arg, struct udp_pcb *pcb, struct pbuf *p, const ip_addr_t *recv_addr, u16_t port)
{
    ...
    if(p->tot_len == p->len)
    {
        ...
        ret = airkiss_lan_recv(p->payload, p->tot_len, &airkiss_cfg_ptr);
    }
    else
    {
        ...
        if(lan_buf)
        {
            ...
            ret = airkiss_lan_recv(lan_buf, p->tot_len, &airkiss_cfg_ptr);
        }
    }
    ...
    case AIRKISS_LAN_SSDP_REQ:
    {
        if(get_mqtt_status() == TURING_MQTT_ONLINE)
        {
            ...
            packret = airkiss_lan_pack(AIRKISS_LAN_SSDP_RESP_CMD, (void *)DEVICE_TYPE, (void *)deviceId,
                                       0, 0, lan_buf,
                                       &lan_buf_len, &airkiss_cfg_ptr);
