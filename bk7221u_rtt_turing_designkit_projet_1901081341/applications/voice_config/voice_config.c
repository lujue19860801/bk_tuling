...
static voice_config_callback voice_callback = RT_NULL;
...
static void cmd_voice_config_thread(void *parameter)
{
	...
	if(voice_callback){
		voice_callback(res,result.ssid,result.passwd,result.custom);
	}
...
_err:
    ...
    tid = RT_NULL;
	voice_callback = RT_NULL;
    return;
}

int start_voice_config(voice_config_callback cb)
{
    if (tid)
    {
