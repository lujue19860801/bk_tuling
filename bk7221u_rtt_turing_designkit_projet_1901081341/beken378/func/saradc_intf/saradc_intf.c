...
typedef struct _tadc_entity_
{
    ...
    char tadc_stack[TURING_ADC_TASK_STACK_SIZE];
...
void saradc_work_create(void)
{
	...
	result = rt_thread_init(&tadc_entity->tadc_thread, TURING_ADC_TAG,
							tadc_thread_entry, NULL,
							&tadc_entity->tadc_stack[0],
							sizeof(tadc_entity->tadc_stack),
							TURING_ADC_TASK_PRIORITY, 10);
	if(RT_EOK != result)
	{
		TADC_FATAL_PRINTF("turing_adc_create exceptional\r\n");
		goto create_exit;
    ...
    if(RT_EOK != result)
    {
		TADC_FATAL_PRINTF("rt_thread_startup exceptional\r\n");
		goto create_exit;
	}
