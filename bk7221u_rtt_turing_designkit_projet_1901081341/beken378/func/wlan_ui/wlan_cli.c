...
beken_semaphore_t log_rx_interrupt_sema;
...
static void flash_command_test(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	{
		os_printf("FLASH <R/W/E/M/N> <start_addr> <len>\r\n");
	}
}
...
static const struct cli_command built_ins[] =
{
    ...
    {"reboot", "reboot system by wlan", reboot},
...
#if CFG_IPERF_TEST
extern int iperf_client(const char* server, int number);
static void iperf(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    const char* server;
    int num_client;
    ...
    if(argc != 3)
    {
        os_printf("Usage: iperf 192.168.1.123 1\n");
        return;
    }
    ...
    iperf_client(server, num_client);
}
