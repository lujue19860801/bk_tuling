...
void demo_softap_app_init(char *ap_ssid, char *ap_key)
{
    ...
    os_strcpy((char *)wNetConfig.dnsServer_ip_addr, WLAN_DEFAULT_IP);
...
void demo_wlan_app_init(VIF_ADDCFG_PTR cfg)
{
    ...
    if(cfg->wlan_role == Station) {
        if(cfg->adv == 1) {
        	...
        	networkadv_cfg.dhcpMode = DHCP_Client;
        	...
        	network_cfg.dhcpMode = DHCP_Client;
...
void demo_state_app_init(void)
{
    ...
    if( sta_ip_is_start() )
    {
    	...
    	bk_printf("sta:rssi=%d,ssid=%s,bssid=" MACSTR ",channel=%d,cipher_type:",
    		linkStatus.wifi_strength, linkStatus.ssid, MAC2STR(linkStatus.bssid), linkStatus.channel);
    ...
    if( uap_ip_is_start() )
    {
        ...
        bk_printf("softap:ssid=%s,channel=%d,dhcp=%d,cipher_type:",
    		ap_info.wifi_ssid, ap_info.channel,ap_info.dhcpMode);
        ...
        bk_printf("ip=%s,gate=%s,mask=%s,dns=%s\r\n",
		    ap_info.local_ip_addr, ap_info.gateway_ip_addr, ap_info.net_mask, ap_info.dnsServer_ip_addr);
    }
}
...
void demo_ip_app_init(void)
{
	...
	bk_printf("dhcp=%d ip=%s gate=%s mask=%s mac=" MACSTR "\r\n",
				...
				ipStatus.mask, MAC2STR(ipStatus.mac));
}
