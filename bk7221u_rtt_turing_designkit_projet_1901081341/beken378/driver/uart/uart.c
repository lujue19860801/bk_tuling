...
extern void cli_set_sema(void);
static struct uart_callback_des uart_receive_callback[2];
...
void uart_send_byte(UINT8 uport, UINT8 data)
{
    if(UART1_PORT == uport)
...
/*----------------------------------------------------------------------------
  fputc
 *----------------------------------------------------------------------------*/
int fputc(int uport, FILE *f)
{
    uart_send_byte(UART2_PORT, uport);
...
int uart_is_receive(int uport)
{
    if (UART1_PORT == uport)
        return REG_READ(REG_UART1_INTR_STATUS) & \
                REG_READ(REG_UART1_INTR_ENABLE) & \
                (RX_FIFO_NEED_READ_STA | UART_RX_STOP_END_STA);
    else
        return REG_READ(REG_UART2_INTR_STATUS) & \
                REG_READ(REG_UART2_INTR_ENABLE) & \
                (RX_FIFO_NEED_READ_STA | UART_RX_STOP_END_STA);
}
