...
void rt_hw_stack_print(rt_thread_t thread)
{
    ...
    rt_kprintf("%s code=[0x%x,0x%x],stack=[0x%x,0x%x],sp=0x%x,stackoverflow=%d\n",thread->name, code_start_addr, code_end_addr, stack_start_addr, stack_end_addr, sp, sp < stack_start_addr);
    for (; sp < stack_end_addr; sp += sizeof(size_t))
    {
        pc = *((uint32_t *) sp);
        /* ARM9 using thumb instruction, so pc must be odd */
        if (pc % 2 == 0) {
            continue;
        }
...
void rt_stack_print(int argc, char **argv)
{
    ...
    if (argc < 2)
    {
        rt_kprintf("usage: stack task_name\n");
...
MSH_CMD_EXPORT_ALIAS(rt_stack_print_all, stack_all, rt_stack_print_all);
